    ...
    public class ManageController : Controller
    {
        ...
        public async Task<IActionResult> Login(string username,string psswd) {
            ...
            if (doc != null)
            {
                ...
                //Permiso si una persoan es añadida por el administrador definitivo

                if (HttpContext.Request.Cookies.Keys.Contains("EspecialInsertado")) {

                    int idD = int.Parse(HttpContext.Request.Cookies["EspecialInsertado"]);

                    Doctor doctor = this.repo.FindDoctor(idD);

                    identity.AddClaim(new Claim("Especial", doctor.IdDoctor.ToString()));
